...
it 'returns "fizzbuzz" when passed a multiple of both 3 and 5' do	
	(1..50).each do |i| #loop on the inside will only return one test example (1 success or 1 failure)
		if i % 3 == 0 && i % 5 == 0
...
(1..50).each do |i| #keeping the each loop on the outside produces more test examples
